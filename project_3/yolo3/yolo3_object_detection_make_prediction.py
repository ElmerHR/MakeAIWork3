...
def draw_boxes(path, filename, v_boxes, v_labels, v_scores):
	# load the image
	data = pyplot.imread(os.path.join(path, filename))
	# plot the image
	pyplot.imshow(data)
	...
	if v_scores:
		...
		label = "%s (%.3f)" % (v_labels[idx], v_scores[idx])
		pyplot.text(x1, y1, label, color='white')
	# show the plot
	pyplot.savefig(os.path.join(path, 'bounding_boxes', filename))
	pyplot.close()
...
path_train = '/home/elmer/dev_debian/working/python3/MakeAIWork3/project_3/apple_disease_classification/yolov3/train'
path_test = '/home/elmer/dev_debian/working/python3/MakeAIWork3/project_3/apple_disease_classification/yolov3/test'

paths = [path_train, path_test]

# list to save bounding boxes to
to_csv = []
# copy files
for path in paths:
	for filename in os.listdir(path):
		if os.path.splitext(filename)[-1] == '.jpg':
			...
			image, image_w, image_h = load_image_pixels(full_path, (input_w, input_h))
			# make prediction
